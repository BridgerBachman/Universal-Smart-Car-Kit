...
#all delays for each command were modified for the specific make and model of the car used. 2002 Lexus IS300
import re
import serial
import time
import logging
...
class ELM327:
    ...
    def __init__(self, portname, baudrate, protocol, timeout,
        ...
        # -------------------------- AT RV (read volt) ------------------------
        if check_voltage:
            r = self.__send(b"AT RV",delay=0.05)
    ...
    def auto_protocol(self):
        ...
        r = self.__send(b"ATSP0",delay=0.05)
        ...
        r = self.__send(b"ATDPN",delay=0.05)
        if len(r) != 1:
